 ...
 * addTask, mark them as complete, print each task, print them by completion, and search for task. The linked-list is a good
 ...
 * the most important task `can be easily accessible, but this would likely require us to use more than one data structure for completed
 ...
 * This can also be efficient for school task, because we can use method to determine priortiy by due date and classes.
 */
class ToDoList {
	private Node head;
    private Node tail;
	...
	*/
     ...
     * @author Michael
     * Write an appropriate comment:
     * This method starts by checking if the user wants to see the completed task or pending task, then using an if-else,
	...
	/*
     * Write an appropriate comment:
     * This method returns the task count, which is incremented with each added element.
	 */
     ...
     * .equalsIgnoreCase() method is used for this because it is case-sensitive and doesn't 
     * matter. I used the Merge Sort algorithm, because I wanted to challenge myself and 
     * because Merge Sort is a good algorithm for linked list due to the fact that you don't have
     ...
     * and a boolean value for if it is being sorted by description or not. Then using recursion
     * the base case checks if the list passed is either of size one or empty. This is important 
     * because what the recursive method is doing is splitting the list into smaller sub-lists.
     * Then in the else case, it is finding the middle of the given list by using a helper method get
     ...
     * and then we set that node in the original list equal to null effectively splitting the list into two
     * left and right sublist. Then we call the mergeSort() method again but this time for the first half(left) 
     * and second half(right) of the linked list. Then after these are iterated through the merge method is then used
     * to merge the small sublist of size 1, then the next sublist size 2, all the way up until size n when
     * there is just one list left, our sorted list. I used the overridden compareTo() to sort the description 
